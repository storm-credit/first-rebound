...
[G15AJ 실명 급여 원장](../research/O15G15AJ_DETROIT_AUG6_NAMED_SALARY_LEDGER.md)은 AG CLI가 공식 2021–22 cap/room MLE 기사 수치를 반환한 범위, NotebookLM CLI의 기존 CBA 요약/캡 두 소스가 `RFA greater-of QO` 명문을 담지 않아 **불충분**하다고 응답한 범위, Codex가 NBA–NBPA CBA 전체본과 구단 원문·2차 계약표를 직접 분리한 범위를 기록한다. Claude의 문서 단독 반증은 [검토 기록](../reviews/R01_O15G15AJ_SALARY_CLI_AND_BLIND.md)에서 수용/기각한다. 같은 NBA 발표에 대한 AG/NLM/Codex 접근을 독립 출처 여러 건으로 세지 않으며, 부분합 검사기 통과를 완전 Team Salary나 G16/G17 PASS로 승격하지 않는다.

[G15AK 권리·웨이버 날짜](../research/O15G15AK_DETROIT_AUG6_RIGHTS_AND_WAIVER_TIMING.md)는 Antigravity CLI의 Detroit 8/12 URL 요청이 headless `RunCommand` 허가 거절·최종 답변 빈 문자열로 끝나 **직접 증거 0건**인 점을 기록한다. NotebookLM CLI는 같은 구단 URL 새 소스 추가 실패 후 이미 있던 NBA 8/1 QO 기사 **한 출처**만 제한 질의해 Diallo/Frank 명단과 그 날짜 한계만 답했다. Codex는 Detroit 구단 8/12 검색 가능한 본문·RealGM 거래 날짜·2017 CBA 투웨이 QO 조항을 직접 대조했다. Claude의 무도구 문서 단독 반증은 [검토 기록](../reviews/R01_O15G15AK_RIGHTS_CLI_AND_BLIND.md)에서 별도 판정한다. 실패한 URL 호출을 성공한 Evidence Pack으로, 같은 NBA 기사 재독을 독립 원자료 증가로 세지 않는다. G16/G17·게이트 불변.

[G15AL Olynyk 전 미충원 차지/순서](../research/O15G15AL_DETROIT_PRE_OLYNYK_ROSTER_CHARGE_SEQUENCE.md)는 G15AJ의 모든 사건 후 실명 소계와 **더 이른 계약 직전** Team Salary 후보를 구분했다. Antigravity CLI의 NBA 자유계약 설명 URL 요청은 headless `RunCommand` 거부·본문 0건, NotebookLM CLI는 기존 2018 NBA CBA 요약 한 소스가 12명 미달 규칙과 2021–22 최소급을 담지 않는다고 답했다. Codex는 2017 CBA §4(f) 전체본·공식 cap과 2차 급여표를 분리해 11명 P0-B의 미충원 차지 한 자리 및 서명 순서 민감도를 계산했다. Claude 문서 단독 반증은 [검토 기록](../reviews/R01_O15G15AL_ROSTER_CHARGE_CLI_AND_BLIND.md)에 수용/기각을 남긴다. 작은 명목 여지를 실제 계약 적격성이나 작가확정으로 승격하지 않으며 G16/G17·게이트 불변.

[G15AM Antigravity 읽기 전용 복구](../reviews/O15G15AM_ANTIGRAVITY_READONLY_RECOVERY.md)는 **별도 재시험**에서 `read_url_content→view_file`을 명시하고 `run_command`를 금지해 공식 NBA cap 기사를 읽었다. stream-json 이벤트·저장 본문의 `$112.414m`/`$4.910m`·`denied_actions=null`을 확인했다. 기존 G15AL 실패는 그대로 남기며 이 성공을 같은 공식 기사에 대한 새 **독립 원자료**로 세지 않는다. 향후 headless URL 수집은 전역 명령 권한을 넓히기 전에 이 도구 경로를 먼저 사용하고, 실행 상태 문자열만이 아니라 실제 도구 이벤트와 본문을 검사한다. 연구·설계/원고 게이트 불변.

[G15AN FA·웨이버/예외 분기](../research/O15G15AN_DETROIT_WAIVER_AND_EXCEPTION_BRANCHES.md)는 NBA 드래프트 프로필의 11계약/5FA를 시점이 명시된 이름 목록으로 사용하고, NBA의 뒤이은 Smith Jr. QO 미제출을 적용해 프로필 분류의 날짜 한계를 보였다. Antigravity는 읽기 전용 도구로 Detroit 팀 기사 두 URL에 접근했으나 본문 없는 HTML 셸이라 **본문 증거 0건**; NotebookLM은 NBA 프로필과 2017 CBA 전체 PDF를 새 소스로 추가해 각각 한 출처 질의를 수행했다. 같은 공식 자료의 AG/NLM/Codex 재독을 독립 원자료로 세지 않는다. Codex는 구단이 설명한 McGruder 비보장 `$5m`를 cap 장부에 남기는 경우/제거하는 경우와 CBA §4(f) 인원 변화를 별도 계산했다. CBA의 48시간 웨이버 기간과 §6(m)(2) 예외 차지를 확인했지만 **웨이버 중 비보장 급여 차지 제거 시각**과 실제 예외 포기는 `HOLD`; Claude 문서 단독 검토는 [별도 기록](../reviews/R01_O15G15AN_WAIVER_AND_EXCEPTION_BLIND.md)에 둔다. G16/G17 및 정본/게이트 불변.

[G15AO 합의 통지 시점](../research/O15G15AO_DETROIT_DISCLOSED_AGREEMENT_CAP_TIMING.md)은 2017 CBA §13(a)(i)–(ii)·§15·§4(a)(1)(iv)·§4(f)(2)와 NBA 공식 2021 모라토리엄 날짜를 직접 대조한다. NotebookLM CLI는 앞서 업로드한 CBA PDF **한 출처**에 한정 질의해 같은 조항과 실제 Detroit 접수시각 부재를 확인했다. Antigravity의 Detroit 공식 기사 본문 0건은 G15AN 기록을 그대로 유지하고 이번에 성공으로 재계수하지 않는다. Claude CLI의 문서 단독 반증이 지적한 미충원 차지 표기는 [검토 기록](../reviews/R01_O15G15AO_DISCLOSED_AGREEMENT_BLIND.md)에 수용/기각을 구분했다. 서명보다 앞선 통지 급여 후보를 추가하지만 실제 팀 캡 적격성·P0-B·G16/G17, 정본/게이트는 `HOLD`다.

[G15AP Lee FA Amount 정정](../research/O15G15AP_SABEN_LEE_HOLD_AND_SEQUENCE_CORRECTION.md)은 Antigravity CLI `read_url_content→view_file`의 공식 NBA QO 기사 **저장 본문**에서 Lee 제한적 투웨이 FA를 확인하고, NotebookLM CLI의 업로드된 2017 CBA PDF **한 출처** 질의와 Codex CBA 직접 독해로 §4(d)(7) FA Amount·§4(f)(2) 인원을 적용했다. 도구 세 개가 같은 두 원자료를 재독했으므로 독립 원자료 세 건으로 세지 않는다. P0-B의 기존 미충원 한 자리를 Lee FA Amount로 바꿔 총액은 유지하고 인원/후속 계약 산술을 정정했다. Claude CLI `haiku --restricted --tools ''`에 결과 문서만 제공한 [반증 기록](../reviews/R01_O15G15AP_LEE_HOLD_BLIND.md)은 표현/전제 위험을 수용·기각으로 분리했다. 이전 G15AL/G15AN/G15AO 숫자 중 미충원 차지 제거에 의존한 값은 G15AP로 대체한다. 실제 권리·리그 접수, 독립 공식 급여, 전체 장부는 `HOLD`; 정본과 게이트 불변.

[G15AQ Lee 계약 수단과 연속 부족](../research/O15G15AQ_DETROIT_LEE_CAP_ROOM_AND_NONBIRD_ROUTE.md)은 Antigravity CLI에서 NBA 자유계약 설명 **본문 성공**, Detroit Lee 회고 **HTML 셸/본문 실패**를 URL별로 기록했다. NotebookLM CLI는 업로드된 CBA PDF **한 출처**에만 §6(b)(2)·Article IX §1을 질의했다. Codex는 같은 CBA 및 공식 NBA CBA 101을 대조해 Non-Bird 3년 계약의 규칙상 가능성만 확인했다. SalarySwish/HoopsRumors의 `Lee Cap Room` 분류는 2차 기록으로 분리했고, P0-B에서 이 수단을 유지하는 조건의 누적 부족을 [검사기](../tools/check_o15g15aq_detroit_lee_route.py)로 계산했다. Claude 문서 단독 반증은 [별도 기록](../reviews/R01_O15G15AQ_LEE_ROUTE_BLIND.md)에 둔다. 동일 원자료 재독·실패한 URL을 독립 증거로 계수하지 않으며 실제 리그 cap sheet·순서·예외 수단, 정본·게이트는 `HOLD`다.

[G15AR 실명 이탈과 미충원 차지](../research/O15G15AR_DETROIT_NAMED_EXIT_AND_ROSTER_CHARGE.md)는 Antigravity CLI `read_url_content→view_file`로 NBA 9월 Nets–Detroit 기사 **본문을 직접 회수**했다. 9월 Jordan/픽 네 장/현금의 실제 거래를 8월 순급여 0 거래로 오인하지 않는다. NotebookLM CLI는 업로드된 CBA PDF **한 출처**의 §4(f) 인원·미충원 차지 사건을 재독했고, Codex는 공식 NBA 9/4·10/6 거래표와 2차 급여를 분리해 [사건별 검사기](../tools/check_o15g15ar_detroit_named_exit.py)를 실행했다. Claude 문서 단독 반증은 [R01](../reviews/R01_O15G15AR_NAMED_EXIT_BLIND.md)에 수용/기각을 기록한다. 동일 NBA/CBA 자료의 여러 도구 접근을 독립 원자료 증가로 세지 않고, 실제 8/6 수취팀·픽·cap 적격성과 정본/게이트는 `HOLD`다.

[G15AS Houston 예외·픽 시간 경계](../research/O15G15AS_HOUSTON_AUG6_TPE_AND_PICK_GATE.md)는 실제 8/7 Theis 영입과 10/6 Sekou 수취를 분리한다. Antigravity CLI는 Rockets 공식 기사에 `read_url_content→view_file`을 실행했지만 HTML 셸만 회수해 **본문 증거 0건**; 그 CLI의 역사 설명은 추론으로 제외했다. NotebookLM CLI는 업로드된 2017 CBA PDF **한 출처**에서 §6(j)(1)(ii)의 한 거래 예외 급여 합계를 재독했고 Codex가 원문을 직접 대조했다. ESPN의 Oladipo 예외 사용 계획과 두 2차 급여 후보를 합산하면 같은 예외로 Sekou+Theis를 모두 받는 가설은 한도 초과이나, 별도 예외·다른 거래 구조를 기각하는 증거는 아니다. Claude 문서 단독 반증은 [R01](../reviews/R01_O15G15AS_HOUSTON_BLIND.md)에 기록한다. 같은 CBA 재독과 실패한 AG URL을 독립 원자료 증가로 세지 않는다. 8/6 Houston 예외 장부·Detroit 대가·픽 소유/보호·원역사 9월/10월 연쇄와 정본/게이트는 `HOLD`다.

[G15AT Houston 별도 예외 수치 후보](../research/O15G15AT_HOUSTON_SEPARATE_HARDEN_EXCEPTION_SCREEN.md)는 6월 2차 예산표의 `$5,019,920` 별도 예외와 Bradley 2020–21 급여 `$5,635,000`을 확인해 초기 Harden 예외 `$10,654,920`과의 차액을 재구성한다. Antigravity CLI는 Hoops Rumors URL의 메타데이터만 보고 **기사 본문 0건**이라 예외 금액 증거에서 제외했다. NotebookLM CLI는 해당 6월 예산표와 SalarySwish Bradley 계약표를 새 URL 소스로 추가해 두 소스만 지정 질의했다. Codex는 원문 2차 표·CBA §6(j)를 직접 읽고, 같은 예외 재사용 금지와 **서로 다른 예외에 각 선수 수취** 후보를 분리했다. Claude 문서 단독 반증은 [R01](../reviews/R01_O15G15AT_SEPARATE_TPE_BLIND.md)에 수용/기각을 기록한다. 6월 자료·8월 2차 약액의 재독은 리그 공식 8/6 사용 장부가 아니며, Detroit 대가/명단/후속 거래와 정본/게이트 `HOLD`다.
