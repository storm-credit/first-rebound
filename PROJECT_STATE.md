...
[G15J 계약 입력 원장](research/O15G15J_ORLANDO_2021_22_CONTRACT_INPUT_LEDGER.md)은 원역사 비공식 계약 추적값 Vučević $24,000,000과 Aminu 옵션 $10,183,800을 회수했다. 두 값의 $34,183,800 합계는 **Vučević가 2021–22에도 Orlando에 잔류하고 Aminu가 옵션을 행사해 Orlando가 계약을 보유한다는 조건의 2인 부분합**이다. 공식 NBA 자료는 리그 기준선과 계약 사건일을 뒷받침하지만 선수별 상세 급여를 공개하지 않는다. 방출 뒤 보장급여 잔류, 미행사 시 FA 관련 비용, 거래 시 수취 팀·반대급부를 분리했고 전체 Orlando 팀 샐러리·cap room·세금/거래 합법 판정은 보류했다. 옵션 행사/미행사, Vučević 후속 잔류, Aminu 처리, Lopez/Moritz 후속은 작가확정 0건이다.

NotebookLM CLI는 NBA CBA/캡 공식 두 출처만 지정해 팀 샐러리 규칙과 선수별 금액 부재를 재확인했다. 구단 영입 기사 두 URL의 새 출처 추가는 실패했고, Anti-Gravity CLI도 두 URL에서 기사 본문을 판독하지 못했다. Claude CLI의 두 반증 시도도 응답 없이 중단됐으므로 G15J 독립 반증·source-blind는 `NOT_RUN`이다. 실패를 증거 확보나 검수 완료로 세지 않는다. 다음은 **2021년 5월 대체 Orlando Aminu 옵션과 7–10월 선수별 계약·FA 권리·드래프트/거래·예외 원장**을 날짜별로 채우는 것이다. G14 ORL4 `ROLE_HOLD`·DET4 `PRIOR_HOLD`, G16/G17 미완료, 전체 7개 매크로 게이트 1완료·1진행·5대기/남은6개, `PROJECT_FREEZE v0.30 PARTIAL`, 설계/원고 `CLOSED`, author/season/exact/manuscript false 유지.

## O-15G15K — Aminu 옵션 보도일·통지일 교정과 작업상 우선 후보
...
## O-15G15X — Denver 1/23 앞코트 수신자 후보

[G15W 두 경기 게이트](research/O15G15W_2022_01_23_TWO_GAME_OPPORTUNITY_GATE.md)의 다음 좁은 빈칸을 [G15X](research/O15G15X_DENVER_JAN23_FRONTCOURT_RECEIVER.md)에서 조사했다. NBA 19:30 ET 공식 부상 보고는 원역사 JaMychal Green `Available`, Jeff Green·Čančar·Porter Jr. `Out`이고, 최종 박스는 JaMychal `DNP - Coach's Decision`이다. 이전 경기 요약의 JaMychal 방역 결장 서술을 최종 `Out` 근거로 쓰지 않는다. Denver가 Nnaji를 Orlando에 보내는 조건에서, Gordon31:15+Jokić36:06+Cousins11:49=79:10이므로 C+PF 96분에는 최소 16:50이 더 필요하다. **후보 X1**은 JaMychal PF16:45, Cousins C+0:05, Reed−0:05로 앞코트 각48분과 팀240분을 맞춘다. 계약·의료·코치 선택·실제 5인 교대/공격 예산은 미증명이다. NotebookLM CLI는 공식 PDF를 한정 질의해 상태 행을 재확인했고, Anti-Gravity CLI는 구단 기사 본문 접근 실패, Claude/source-blind는 산술만 통과한 제한 검토다. G16/G17 완료로 세지 않는다. 다음은 X1의 Denver 계약/자리·동시 교대와 Detroit 대체 명단 맞대조다. Chicago 2020–21 정확 시즌, G14 DET/ORL `HOLD`, 시즌/작가/원고 미확정. 전체 7개 매크로 게이트 1완료·1진행·5대기/남은6개, `PROJECT_FREEZE v0.30 PARTIAL`, 설계/원고 `CLOSED` 유지.

## O-15G15Y — 1/23 세 경기·Bey 상대 전환

[G15Y 날짜 원장](research/O15G15Y_2022_01_23_THREE_GAME_PLAYER_FLOW.md)은 G15W의 두 경기 외 `ATL@CHA`를 추가했다. 2020 정본 Bey DEN22·LaMelo CHI4, 2021 DB1 Cade CHA1·Suggs DET5 후보, Gordon A의 Nnaji ORL 조건을 같은 미국 현지 1/23에 놓는다. 원역사 Detroit Bey30:42+Cade36:14는 합계66:56·26 FGA·29점·9 TOV이지만 대체 Detroit 공격 예산은 아니다. 원역사 ORL Suggs32:46, CHA LaMelo33:58도 새 팀으로 기록을 그대로 옮기지 않는다. Bey가 2022년 Denver에 잔류하는 조건이면 G15X의 JaMychal X1과 **Bey PF X2**가 수신자 경쟁안이다. 둘 다 5인 교대·계약·의료·상대 공격기회가 `HOLD`다. 다음은 Denver X1/X2의 5인/자리와 Detroit Patrick/Kira/Suggs 당일 명단, Charlotte Edwards/Cade 후보를 검증한다. 시즌·작가·원고 신규 확정 0건, G14 DET/ORL `HOLD`, G16/G17 미완료. 전체 7개 매크로 게이트 1완료·1진행·5대기/남은6개, `PROJECT_FREEZE v0.30 PARTIAL`, 설계/원고 `CLOSED` 유지.

## O-15G15Z — Denver 추상 5인 증명·Bey 계약 다리

[G15Z 증명](research/O15G15Z_DENVER_FIVE_MAN_AND_BEY_CONTRACT_BRIDGE.md)과 [기계 검증기](tools/check_o15g15z_denver_lineup.py)는 1/23 Denver X1 JaMychal/X2 Bey의 상호 배타적 앞코트 수신자 각각에 대해 **5개 자리 각48분·매 구간 서로 다른 5인·팀240분**을 재현했다. Cousins +0:05/Reed −0:05를 숨기지 않았다. 이는 연속36:06 Jokić, 실험적 Campazzo SG/Rivers SF/Bey PF를 허용한 **추상 가능성**이며 실제 4쿼터 교대나 농구 적합성 PASS가 아니다. NBA CBA 101에 따라 Bey가 2020 신인 계약에 실제 서명했다면 2021–22는 2년차 보장 구간이지만 2020 지명/급여 시뮬레이션만으로 당일 Denver 보유·등록·건강은 확정되지 않는다. Antigravity CLI는 이번에 NBA 규칙 기사 본문을 판독했고 NotebookLM CLI는 기존 NBA CBA 출처 한정 질의로 계약 2+1+1을 재확인했다. 동일 규칙의 중복 검증을 독립 출처 수로 세지 않는다. 다음은 날짜별 Denver 계약/자리와 현실적인 쿼터 교대, Detroit 대체 명단·공격기회다. Chicago 2020–21 정확 시즌, G14 DET/ORL, G16/G17은 `HOLD`; 신규 작가확정 0건. 전체 7개 매크로 게이트 1완료·1진행·5대기/남은6개, `PROJECT_FREEZE v0.30 PARTIAL`, 설계/원고 `CLOSED`, author/season/exact/manuscript false 유지.
...
## O-15G15AD — Plumlee 보장 계약과 Olynyk cap 경로의 동시 보유 게이트

[G15AD](research/O15G15AD_PLUMLEE_OLYNYK_CAP_AND_DATE_BRIDGE.md)는 Plumlee의 원계약 2021–22 보장 구간을 확인하고, Detroit 구단 자료가 **원역사 Plumlee 처분으로 Olynyk 영입용 cap space를 만들었다**고 설명하는 점을 G14의 조건부 `Plumlee C24 + Olynyk PF18`에 연결했다. P0-A Plumlee 유지/Olynyk 미취득, P0-B 두 선수 동시 보유를 위한 별도 급여·자리·거래 경로, P0-C 후속 Plumlee 이탈을 구별한다. 원역사 8/6 거래 미실행만으로 Olynyk 계약과 Plumlee의 1/23 보유·활동을 같이 확정할 수 없다. `$16.1m`은 Plumlee의 **첫 두 해 합계**이며 2021–22 개인 연봉이나 대체 Detroit 팀 급여가 아니다. Antigravity는 구단 기사 셸만 읽었고 NotebookLM의 새 URL 수집은 실패했다. 기존 공식 거래·CBA 출처 한정 분석에서 DB1에 Thor 권리를 잘못 이식한 부분을 기각했으며 Claude 결과물 단독 의심은 [검토 기록](reviews/R01_O15G15AD_CAP_CLI_AND_SOURCE_BLIND.md)대로 `HOLD`/표현 보강에 반영했다. 다음은 2021-07-29→08-06→10-20→2022-01-23 Detroit 이름별 계약·급여·표준 자리와 Charlotte 센터 대체를 대조한다. Chicago 2020–21 정확 시즌, G14 DET/ORL, G16/G17은 `HOLD`; 신규 작가확정 0건, 전체 7개 게이트 1완료·1진행·5대기/남은6개, `PROJECT_FREEZE v0.30 PARTIAL`, 설계/원고 `CLOSED`, author/season/exact/manuscript false 유지.
...
## O-15G15AL — Detroit 8/6 Olynyk 전 미충원 차지와 서명 순서

[G15AL 조건부 원장](research/O15G15AL_DETROIT_PRE_OLYNYK_ROSTER_CHARGE_SEQUENCE.md)은 2017 CBA §4(f)의 오프시즌 12명 미달 차지를 G15AJ **사건 후 소계와 다른 시점**에 적용했다. 선택하지 않은 P0-B Suggs5·Plumlee 잔류·Patrick7/Kira16, Frank/Diallo QO 존속, 권리 포기/웨이버 선행, 2차 급여표를 모두 가정하면 Olynyk 전 11명·미충원 한 자리, 조건부 Team Salary `$100,052,228`, 명목 잔여 `$12,361,772`다. Olynyk `$12,195,122` 먼저면 계약 직전 여지 `$166,650`·서명 후 미충원 차지 제거까지 반영한 잔여 `$1,091,908`; Lyles 먼저면 Olynyk 직전 `$1,408,092` 부족이다. 방출 잔액 표만 바꾸어도 Olynyk 먼저 분기가 `$36,921` 부족으로 뒤집혀 **실제 캡룸/동일 사건 실행 판정은 HOLD**다. [검사기](tools/check_o15g15al_detroit_roster_charge.py)는 G15AJ와 조건부 산술만 대조하고 [도구·제한 반증 기록](reviews/R01_O15G15AL_ROSTER_CHARGE_CLI_AND_BLIND.md)은 Antigravity 직접 증거0·NotebookLM 요약 소스 부재 확인·Claude 문서 단독 지적을 구분한다. 다음은 NBA 계약 접수 순서·완전 Team Salary·나머지 같은 날 계약과 8/10 Joseph 예외, 필요시 이름 있는 cap 이탈 거래다. Chicago 2020–21 정확 시즌, G14 DET/ORL, G16/G17은 `HOLD`; 신규 작가확정 0건, 7개 게이트 1완료·1진행·5대기/남은6개, `PROJECT_FREEZE v0.30 PARTIAL`, 설계/원고 `CLOSED` 유지.

## O-15G15AM — Antigravity 직접 기사 읽기 복구

[G15AM 실행 기록](reviews/O15G15AM_ANTIGRAVITY_READONLY_RECOVERY.md)은 별도 Antigravity CLI 재시험에서 `read_url_content→view_file` 이벤트와 저장된 NBA cap 기사 본문을 확인했다. 공개 기사 읽기는 성공했으며 `$112.414m` cap·`$4.910m` room MLE가 원문과 맞는다. G15AL의 headless `RunCommand` 거부는 해당 호출의 실패로 그대로 남긴다. 동일 공식 기사의 재접근을 독립 원자료나 Detroit 전체 cap 검증으로 부풀리지 않는다. 전역 명령 허용 설정과 정본/게이트는 변경하지 않았다. 다음은 같은 읽기 전용 경로로 남은 원문을 회수해 전체 Team Salary·서명 순서·실명 거래를 검증한다. 전체 7개 게이트 1완료·1진행·5대기/남은6개, `PROJECT_FREEZE v0.30 PARTIAL`, 설계/원고 `CLOSED` 유지.
...
## O-15G15AS — Houston 수취 예외·픽 날짜 검증

[G15AS Houston 검증 원장](research/O15G15AS_HOUSTON_AUG6_TPE_AND_PICK_GATE.md)은 10/6 Brooklyn→Houston Sekou 거래를 8/6 Detroit→Houston 거래 가능성으로 소급하지 않는다. Houston의 공식 8/7 Theis 사인 앤드 트레이드와 ESPN의 8/2 Oladipo 거래 예외 사용 계획을 연결했다. Sekou `$3,613,680`와 Theis `$8,280,351` 2차 급여 후보의 합계 `$11,894,031`은 보도된 예외 약 `$8.28m`를 넘으므로 **같은 예외 하나로 두 계약을 받는 경로는 기각**한다. 10월 Brooklyn 자체 2024 픽은 9월 Detroit가 받은 Washington 경유 2024 픽이 아니며, Houston의 10월 수취 예외가 8/6에 남아 있었는지도 `HOLD`다. Antigravity CLI의 Rockets 기사 요청은 HTML 셸/본문 0건이라 증거로 세지 않았고, NotebookLM CLI의 2017 CBA 한 출처 답변은 원문 §6(j)(1)(ii)와 대조했다. Claude 문서 단독 반증은 [R01](reviews/R01_O15G15AS_HOUSTON_BLIND.md)에 별도 기록한다. 실명 상대·당시 예외/대가/명단과 Theis·Nets·Jordan·후속 계약의 전체 재계산 없이는 거래 확정 불가. 신규 작가확정 0건, Chicago 정확 시즌/G14/G16/G17·`PROJECT_FREEZE v0.30 PARTIAL`·설계/원고 `CLOSED` 유지. 7행 1완료·1진행·5대기, 남은 큰 작업 6개.

## O-15G15AT — Houston 별도 Harden 예외 수단 후보

[G15AT 별도 예외 원장](research/O15G15AT_HOUSTON_SEPARATE_HARDEN_EXCEPTION_SCREEN.md)은 G15AS의 단일 Oladipo 예외 초과 판정과 별개로 2021년 6월 Houston의 **두 거래 예외 `$8,180,351`/`$5,019,920`**를 확인한다. Hoops Rumors의 Harden 초기 `$10,654,920`에서 Bradley **2020–21** 급여 `$5,635,000`을 차감하면 6월 표의 `$5,019,920`과 일치하며, Sekou 2021–22 급여 후보 `$3,613,680`을 별도 예외로 받는다면 `$1,406,240`이 남는다. Bradley **2021–22 미행사 옵션 `$5,916,750`**을 3월 거래에 쓰는 앞선 중간 계산은 기각했다. 이 배정은 2차 수치의 재구성이고 8/6 리그 예외 사용 내역 원본은 아니다. 8/7 Theis는 Oladipo 예외로 처리할 수 있다는 조건부 수치 경로가 생겼지만 실제 거래 대가·명단·Team Salary·apron, 9/4 Nets 거래 소실과 Detroit Frank/Joseph 이후 부족은 `HOLD`. Antigravity/NotebookLM/Claude의 [검증 기록](reviews/R01_O15G15AT_SEPARATE_TPE_BLIND.md)은 도구별 원문 접근과 과대판정을 분리한다. 신규 작가확정 0건, Chicago 정확 시즌/G14/G16/G17·`PROJECT_FREEZE v0.30 PARTIAL`·설계/원고 `CLOSED` 유지. 7행 1완료·1진행·5대기, 남은 큰 작업 6개.

## O-15G15AU — Detroit 8/6 대가 픽·현금 소유 검문

[G15AU 자산 원장](research/O15G15AU_DETROIT_AUG6_CONSIDERATION_LEDGER.md)은 Houston 별도 예외의 수치상 가능성[G15AT] 다음에 Detroit가 줄 수 있는 대가를 날짜별로 좁혔다. 공식 구단 발표 기준 Detroit 자체 2023 픽은 2018 Philadelphia 송출 후 2022 재취득, 자체 2024·2025·2026 및 Portland 2023 픽은 2020 Clippers 송출, Houston 경유 2027 픽은 2021-03 OKC 송출이므로 2021-08-06 가상 거래에 재사용할 수 없다. Detroit 2022 픽은 후행 #46 사용 기록이 있는 **검토 후보**이나 8/6 소유·보호 조건 원본은 미확보이고 대체 세계에서는 #46 순번을 고정하지 않는다. CBA §8(a)의 거래 현금 규칙과 2021–22 한도 `$5.785m`(2차 추적)은 확인했지만 Detroit의 실제 8/6 잔여 송출액·Houston 요구액은 `HOLD`. 9/4 Brooklyn에서 **나중에 받는** Jordan·픽 네 장·현금도 8/6 자산이 아니다. Antigravity CLI는 빈 응답, NotebookLM CLI는 CBA와 NBA cap 공식 소스 한정 질의에 성공했으나 Detroit 현금 내역은 없다고 답했다. Claude 문서 단독 반증은 [R01 제한 검토](reviews/R01_O15G15AU_CONSIDERATION_BLIND.md)에 기록했고, 현금 한도 계산의 설명 부족만 수용·보강했다. Houston 8/6 예외 원본·명단/Team Salary/apron·양측 대가·Detroit Frank 이후 부족과 9월 Nets/10월 Brooklyn 연쇄는 미확정. 신규 작가확정 0건, Chicago 정확 시즌/G14/G16/G17·`PROJECT_FREEZE v0.30 PARTIAL`·설계/원고 `CLOSED` 유지. 7행 1완료·1진행·5대기, 진행 중 포함 남은 큰 작업 6개.
