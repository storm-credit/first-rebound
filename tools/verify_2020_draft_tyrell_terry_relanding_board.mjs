...
const expected = [['32', 'Charlotte Hornets', 'Tyrell Terry', 'PRIMARY_LEAN_AUTHOR_GATE']];
...
for (const marker of [
  'PICKS_1_TO_31_AUTHOR_LOCKED / PICK_32_AUTHOR_GATE',
  'Tyrell Terry 32',
...
if (!review.includes('PICK_32_AUTHOR_GATE')) throw new Error('missing pick 32 author gate');
if (!review.includes('Graham·Rozier를 보존한다')) throw new Error('missing Charlotte roster firewall');
if (!review.includes('정확 32순위는 작가 승인 전 `HOLD`')) throw new Error('missing exact-pick hold');

console.log('PASS Tyrell Terry relanding board');
console.log('Terry 32 author-gated; Carey relanding conditional');
