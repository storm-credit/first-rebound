...
- Atlanta 주인공의 경기별 분·가용성은 v0.23에서, 183.1분 실명 수취자는 v0.24에서 잠겼다. v0.25 독립 검토에서 같은 805분 차이·이중계산 금지·interaction 0은 통과했지만 생산성 수축·피로·logit scale 수치는 `CALIBRATION_HOLD`다. 82경기 pB·workload·대체 승패도 `HOLD`다.
...
- Hutchison 없는 Chicago–Washington–Boston 거래는 3팀 5인 구조로 `CONDITIONAL_STRUCTURE_PASS`다. 이전 `6인` 표기는 O-15F5에서 선수 수 오기로 정정했다. 실제 발생은 작가 선택 전 `HOLD`다.
