...
건강·등록·거래의 사실 조건을 통과하고 A1~A3의 채택 기록이 생기기 전에는 2021 동률 추첨과 상위 4픽 추첨을 실행하지 않는다. 먼저 결과를 본 뒤 시즌안이나 seed를 바꾸는 역선택을 막기 위해서다. [드래프트 인과 규약](DRAFT_CAUSALITY_PROTOCOL.md)의 고정 seed를 유지하며 새 seed 후보를 만들지 않는다.
...
현재 규칙은 **사실 조건 통과 → A1~A3 채택 → 고정 추첨 → 보호픽 결산 → 2020–21 종료 → 2021–23**다. 이미 승인한 팀·드래프트·Theis/Green·R1/T1~T4는 재질문하지 않는다.

사용자의 자동 후속 의도를 처리하기 위해 [조건부 결산 제안 CP2](../design/O15F14_CONDITIONAL_CONTINUATION_PROPOSAL.md)를 작성했다. 이는 미확인 사실을 HOLD로 유지하고 잠정 시즌/추첨과 장기 설계를 계속하는 작업 순서 제안이다. 상호 배타적인 네 안 중 추천이며 **미채택**이다. 이 문서의 3절 금지는 아직 유효하고 이번 작업에서 추첨하지 않았다.
